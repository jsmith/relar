...
function getItemData(image: Image, columnWidth: number) {
  ...
  const adjustedHeight = Math.round(columnWidth * imageRatio);
...
function ImageGridList(props: { images: Image[] }) {
  return (
    <GridList
      ...
      renderItem={(image) => {
        return (
          <img src={image.url} width={image.width} height={image.height} className={styles.image} />
        );
      }}
    />
  );
...
export function GridTempTest(props: any) {
  return (
    <div>
      <div className={styles.header}>
        <a href="https://github.com/jamiebuilds/react-gridlist" className={styles.headerLink}>
