...
export class StrictEvent<T extends string> extends Event {
  constructor(type: T) {
    super(type);
  }
}
...
export class StrictEventTarget<T extends string> extends EventTarget {
  addEventListener(
    type: T,
  ...
  dispatchEvent(event: StrictEvent<T>): boolean {
...
const newEventTarget = new StrictEventTarget();
